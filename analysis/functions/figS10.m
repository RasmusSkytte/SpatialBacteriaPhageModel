...
for i = 1:n
    ...
    switch i
        ...
        case 2
            subpaths = dir(sprintf('%s/Model_8',path));
            importPath = sprintf('%s/Model_8/%s/data_Model_8.mat',path,subpaths(end).name);
        case 3
            subpaths = dir(sprintf('%s/Model_4',path));
            importPath = sprintf('%s/Model_4/%s/data_Model_4.mat',path,subpaths(11).name);
        case 4
            subpaths = dir(sprintf('%s/Model_9',path));
            importPath = sprintf('%s/Model_9/%s/data_Model_9.mat',path,subpaths(end).name);
    end
...
% A/B/C/D Label
hA = text(mar+1.5*sep,(n-1)*(hei+sep)+hei+mar,'Model 3','Units','Pixels');
hA.Color = 'k';
hA.FontWeight = 'bold';
hA.FontSize = 16;

hB = text(mar+1.5*sep,(n-2)*(hei+sep)+hei+mar,'Model 3','Units','Pixels');
hB.Color = 'w';
hB.FontWeight = 'bold';
hB.FontSize = 16;

hB = text(mar+1.5*sep,(n-2)*(hei+sep)-sep+hei+mar,'\tau = 0 h','Units','Pixels');
hB.Color = 'w';
hB.FontWeight = 'bold';
hB.FontSize = 12;

hC = text(mar+1.5*sep,(n-3)*(hei+sep)+hei+mar,'Model 4','Units','Pixels');
hC.Color = 'k';
hC.FontWeight = 'bold';
hC.FontSize = 16;

hD = text(mar+1.5*sep,(n-4)*(hei+sep)+hei+mar,'Model 4','Units','Pixels');
hD.Color = 'w';
hD.FontWeight = 'bold';
hD.FontSize = 16;

hD = text(mar+1.5*sep,(n-4)*(hei+sep)-1.35*sep+hei+mar,'\tau = 0 h','Units','Pixels');
hD.Color = 'k';
hD.FontWeight = 'bold';
hD.FontSize = 12;
...
print('../Fig_S10/FigS10.tif','-dtiff')