...
% Prepare figure
fh = figure();
fh.Position = [1500 -250 3*wid+2*mar+3*sep+sep 4*hei+2*mar+4*sep];
...
for i = 1:4
    ...
    % Load data
    subpaths = ls(sprintf('%s/Model_%d',path,i));
    subpaths = strsplit(subpaths(1:end-1),' ');
    importPath = sprintf('%s/Model_%d/%s/data_Model_%d.mat',path,i,subpaths{end},i);
...
print('-r300','Fig_4/PhaseSpace_w_contour.tif','-dtiff')
