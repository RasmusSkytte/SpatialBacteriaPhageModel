    ...
    for (int model = 1; model < 12; model++) {
        for (int j = P_0.size()-1; j >= 0; j--) {
            for (int i = B_0.size()-1; i >= 0; i--) {
                ...
                double tau;
                if ((model == 1) or (model == 8) or (model == 9)){
                    tau = 0.0;
                } else  {
                    tau = 0.5;
                }
                ...
                // For models 1 - 4, B0 = 1e4, and P0 = 1e5 is used as example so more output is needed
                if (not ((model < 5) and (B_0[i] == pow(10, 4.00)) and  (P_0[j] == pow(10, 5.00)))) {
                    s.FastExit();
                }
                ...
                if (model == 1) {               // Simple model
                ...
                } else if (model == 9) {        // Model with space and colony level protection but not latency

                    // Set the size of the space
                    s.SetGridSize(nGrid);

                } else if (model == 10) {        // Model with time delay and only a single internal state
                    ...
                    // Use only a single internal state
                    s.SingleInternalState();

                } else if (model == 11) {        // Model with time delay and colony level protection

                    // Set the size of the space
                    s.SetGridSize(1);

                } else if (model == 12) {        // Model with time delay, space, and bacterial diffusion

                    // Set the size of the space
                    s.SetGridSize(nGrid);

                    // Set bacterial diffusion rate
                    s.CellDiffusionConstant(5e2);    //  = D_P / 20
